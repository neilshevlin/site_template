...
const CONSOLE_PRINT = `YOUR PAGE CONSOLE PRINT`;
...
export default function MyApp(props) {
  ...
  const isSecondary = props.router.asPath.startsWith('/secondary')
  const isLandingPage = props.router.pathname === '/';

  React.useEffect(() => console.log(CONSOLE_PRINT), []);
  useGoogleAnalytics();
  ...
  return (
    <div className={`${isLandingPage ? 'page--landing' : ''}`}>
      ...
      <Head>
        ...
        <meta property="og:url" content="YOUR EXAMPLE WEBSITE URL" />
        <meta property="og:title" content={title} />
        <meta property="og:description" content={description} />
        <meta
          property="og:image"
          content="YOUR META IMAGE URL"
        />
        <meta name="twitter:card" content="summary" />
        <meta
          name="twitter:image"
          content="YOUR TWITTER IMAGE URL"
        />
      </Head>
      ...
      <div className="page">
        {isSecondary ? <SideNav /> : null}
        <main className="flex column">
          <div id="skip-nav" />
          <Component {...pageProps} />
        </main>
        {isSecondary && toc ? <TableOfContents toc={toc} /> : null}
      </div>
      ...
      <style jsx global>
        {`
          ...
          main article {
            ${isSecondary ? 'padding: 2rem 1.5rem 3rem;' : ''}
          }

          .footer-bar {
            flex: 1;
            padding: ${isLandingPage ? '1rem 0' : '0'};
            border-top: ${isSecondary ? '1px solid var(--gray-medium)' : 'none'};
          }
